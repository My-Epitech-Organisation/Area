...
export default function EditUserPage({ params }: Props) {
  ...
  if (!user) return <main className="p-6">Not found</main>;

  return (
    <main className="p-6 max-w-xl mx-auto">
      <h1 className="text-2xl font-semibold mb-4">Edit User #{user.id}</h1>
      <form onSubmit={onSave} className="mb-4 flex flex-col gap-2">
        ...
        <div className="flex gap-2 mt-2">
          <button className="border px-4 rounded" type="submit">Save</button>
          <button className="border px-4 rounded" type="button" onClick={onDelete}>Delete</button>
        </div>
      </form>
    </main>
  );
