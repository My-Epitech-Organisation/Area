...
class ServiceActionSelectorCard extends StatefulWidget {
  ...
  final String?
  selectedTriggerAction; // For filtering reactions by compatibility
  ...
  const ServiceActionSelectorCard({
    ...
    this.selectedTriggerAction, // Optional: for reaction filtering
  });
...
class _ServiceActionSelectorCardState extends State<ServiceActionSelectorCard> {
  ...
  /// Filter reactions based on selected trigger action compatibility
  List<DropdownMenuItem<String>>? _getFilteredReactions(
    ServiceCatalogProvider serviceProvider,
    CompatibilityProvider compatibilityProvider,
  ) {
    final allReactions = serviceProvider.getReactionsForService(
    ...
    // If we have a selected trigger action, filter by compatibility
    if (widget.selectedTriggerAction != null &&
        widget.selectedTriggerAction!.isNotEmpty) {
      final compatibleReactionNames = compatibilityProvider
          .getCompatibleReactions(
            widget.selectedTriggerAction!,
            allReactions.map((r) => r.name).toList(),
          );

      final filteredReactions = allReactions
          .where((reaction) => compatibleReactionNames.contains(reaction.name))
          .toList();

      return filteredReactions.map((reaction) {
    ...
    // No filtering if no trigger action selected
