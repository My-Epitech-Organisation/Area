...
class OAuthCallbackView(APIView):
    ...
    def get(self, request, provider: str):
        ...
        try:
            ...
            # Step 4.5: For Notion, fetch and store authorized pages/databases
            if provider == "notion":
                try:
                    self._fetch_and_store_notion_pages(
                        user, token_data["access_token"]
                    )
                except Exception as e:
                    # Don't fail OAuth if page fetching fails
                    logger.error(
                        f"Failed to fetch Notion pages for {user.email}: {str(e)}",
                        exc_info=True,
                    )
    ...
    def _fetch_and_store_notion_pages(self, user, access_token: str):
        """
        Fetch accessible pages and databases from Notion API and store them.

        This implements the GitHub App-like UX where users can select from
        their authorized pages instead of manually entering UUIDs.
        ...
        Flow:
            1. Call Notion /v1/search API to get all accessible pages/databases
            2. Parse response and extract page metadata
            3. Store in NotionPage model for later selection in UI
        """
        import requests

        from automations.models import NotionPage

        try:
            # Call Notion search API to get all accessible pages/databases
            ...
            if not response.ok:
                logger.warning(
                    f"Notion /search API returned {response.status_code}: "
                    f"{response.text}"
                )
                return
            ...
            logger.info(f"Fetched {len(results)} Notion pages for user {user.email}")
            ...
            for result in results:
                ...
                is_database = object_type == "database" or result.get("parent", {}).get(
                    "type"
                ) == "database_id"
                page_type = (
                    NotionPage.PageType.DATABASE
                    if is_database
                    else NotionPage.PageType.PAGE
                )
                ...
                if "title" in result:
                    ...
                    if title_array and isinstance(title_array, list):
                        title = (
                            title_array[0].get("plain_text", "Untitled")
                            if title_array
                            else "Untitled"
                        )
                elif "properties" in result:
                    # For databases - look for Name property
                ...
                # Extract workspace ID from parent or use default
                workspace_id = result.get("workspace_id", "")
                if not workspace_id:
                    # Try to extract from parent
                    parent = result.get("parent", {})
                    workspace_id = parent.get("workspace_id", "default")

                # Build Notion URL
                url = f"https://www.notion.so/{page_id.replace('-', '')}"

                # Store or update page
                page_obj, created = NotionPage.objects.update_or_create(
                    user=user,
                    page_id=page_id,
                    defaults={
                        "page_type": page_type,
            ...
            logger.info(
                f"Notion pages sync complete for {user.email}: "
                f"{pages_created} created, {pages_updated} updated"
            )

        except requests.RequestException as e:
            logger.error(
                f"Network error fetching Notion pages: {str(e)}", exc_info=True
            )
            raise
        except Exception as e:
            logger.error(
                f"Unexpected error fetching Notion pages: {str(e)}", exc_info=True
            )
            raise
