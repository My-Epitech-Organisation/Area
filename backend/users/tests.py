...
class TokenLifecycleTests(TestCase):
    ...
    def test_refresh_token_rotation(self):
        ...
        if 'refresh' in refresh_response.data:
            ...
            old_response = self.client.post(self.refresh_url, old_refresh_data, format='json')
            # This might be 401 if blacklisting is enabled
            self.assertIn(old_response.status_code, [status.HTTP_401_UNAUTHORIZED, status.HTTP_400_BAD_REQUEST])
