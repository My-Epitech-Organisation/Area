...
class GoogleWebhookWatch(models.Model):
    """
    Track active Google push notification subscriptions (watches).

    Google services (Gmail, Calendar, YouTube) use push notifications via
    their API. Each watch has:
    - A unique channel_id (UUID) to identify the subscription
    - A resource_id returned by Google
    - An expiration time (watches must be renewed before expiry)

    Key Features:
    - Automatic watch renewal before expiration
    - Per-user, per-service tracking
    - Support for multiple resources per user (e.g., multiple calendars)
    """
    ...
    user = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        on_delete=models.CASCADE,
        related_name="google_watches",
        help_text="User who owns this watch",
    )

    service = models.CharField(
        max_length=20,
        choices=Service.choices,
        help_text="Google service being watched",
    )
    ...
    channel_id = models.CharField(
        ...
        unique=True,
        db_index=True,
        help_text="Unique UUID identifying this watch (generated by us)",
    )

    resource_id = models.CharField(
        max_length=255,
        help_text="Resource ID returned by Google after creating the watch",
    )
    ...
    resource_uri = models.CharField(
        ...
        blank=True,
        default="",
        help_text="Optional: Specific resource being watched (e.g., calendar ID)",
    )

    expiration = models.DateTimeField(
        help_text="When this watch expires (must renew before this time)",
    )
    ...
    last_event_at = models.DateTimeField(
        null=True,
        blank=True,
        help_text="Timestamp of the last received push notification",
    )
    event_count = models.IntegerField(
        default=0,
        help_text="Total number of push notifications received",
    )
    ...
    def record_event(self):
        """Record that a push notification was received."""
        from django.utils import timezone
